...
def _plain_sgd(cnp.ndarray[double, ndim=1, mode='c'] weights,
    ...
    """
    # encrypt :
    context, secret_key = gaia.key_gen("ckks")
    transfer.public_key.remote(context.serialize(), role='HOST', idx=-1,
                                                    suffix='pub_key')
    ...
    with nogil:
        for epoch in range(max_iter):
            ...
            for i in range(n_samples):
                ...
                p_guest = w.dot(x_data_ptr, x_ind_ptr, xnnz) + intercept

                with gil:
                    p_host = transfer.y_host.get(idx=-1, suffix=f"y_host_{epoch}")
                    p = p_guest + p_host
                ...
                with gil:
                    cipher_delta_y = th.Tensor(update).encrypt("ckks", public_key=context)

                    transfer.cipher_delta_y.remote(cipher_delta_y, 
                                                        role='HOST', 
                                                        idx=-1,
                                                        suffix=f'cipher_delta_y_{epoch}')
